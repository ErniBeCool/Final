...
public class GameManager : MonoBehaviour
{
    ...
    public void StartGame()
    {
        ...
        Debug.Log("Игрок создан: " + player.name + " на позиции " + player.transform.position);
    ...
    private void SpawnEnemy()
    {
        ...
        float spawnDistance = gameData != null ? Random.Range(gameData.MinSpawnOffset, gameData.MaxSpawnOffset) : 15f;
        Vector3 spawnOffset = new Vector3(randomDirection.x * spawnDistance, 0, randomDirection.y * spawnDistance);

        // Определяем высоту земли с помощью raycast
        ...
        {
            spawnPosition.y = hit.point.y; // Устанавливаем высоту на уровне земли
        }
        else
        {
            spawnPosition.y = playerPosition.y; // Если raycast не попал, используем высоту игрока
        }
